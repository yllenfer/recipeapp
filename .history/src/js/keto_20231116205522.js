...
function displayKeto(data) {
    ...
    const recipeHtml = recipes.map(recipe => `
        <div class="recipe">
            <h3>
            <a href="./src/js/recipecontainer.html/${recipe.id}" redirectToRecipe(${recipe.id})" target="_blank">${recipe.title}</a>
            </h3>
        </div>
    `).join('');
...
function redirectToRecipe(recipeId) {
    ...
    const baseUrl = "./src/recipe-display/recipecontainer.html/";
