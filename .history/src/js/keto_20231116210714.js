...
function displayKeto(data) {
    ...
    const recipeHtml = recipes.map(recipe => `
        <div class="recipe">
            <h3>
            <a href="recipe-display/recipecontainer.html/${recipe.id}" redirectToRecipe(${recipe.id})" target="_blank">${recipe.title}</a>
            </h3>
        </div>
    `).join('');
...
function redirectToRecipe(recipeId) {
    const baseUrl = "./recipe-display/recipecontainer.html";
